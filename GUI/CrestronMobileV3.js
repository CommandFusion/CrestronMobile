...
var CrestronMobile = {
	...
	setup:function () {
		...
		CF.getJoin(CF.GlobalTokensJoin, function (j, v, tokens) {
			CrestronMobile.aJoinMax = parseInt(tokens["[aJoinMax]"], 10) || 200;
			CrestronMobile.sJoinMax = parseInt(tokens["[sJoinMax]"], 10) || 200;
			CrestronMobile.dJoinMax = parseInt(tokens["[dJoinMax]"], 10) || 1000;
			CrestronMobile.gJoinMin = parseInt(tokens["[gJoinMin]"], 10) || 1001;
			CrestronMobile.gJoinMax = parseInt(tokens["[gJoinMax]"], 10) || 1050;
			...
			if (CrestronMobile.debug) {
				CF.log("CrestronMobile: dJoinMax=" + CrestronMobile.dJoinMax + ", aJoinMax=" + CrestronMobile.aJoinMax + ", sJoinMax=" + CrestronMobile.sJoinMax);
			}
			...
			CrestronMobile.heartbeatTimer = setInterval(function () {
				if (CrestronMobile.updateComplete) {
					...
					if (++CrestronMobile.heartbeatCount > 5 && !CrestronMobile.loggedIn) {
						if (CrestronMobile.debug) {
							CF.log("CrestronMobile: heartbeatTimer fired, not loggedIn, time to reset connection (heartbeatCount=" + CrestronMobile.heartbeatCount + ")");
						}
	...
	resetRunningJoins:function () {
		...
		for (i = 0, n = CrestronMobile.dJoin.length; i < n; i++) {
			CrestronMobile.dJoin[i] = {
				join:"d" + (i + 1),
				value:0
			}
		}
		for (i = 0, n = CrestronMobile.aJoin.length; i < n; i++) {
			CrestronMobile.aJoin[i] = {
				join:"a" + (i + 1),
				value:0
			}
		}
		for (i = 0, n = CrestronMobile.sJoin.length; i < n; i++) {
			CrestronMobile.sJoin[i] = {
				join:"s" + (i + 1),
				value:""
			}
		}
	...
	sendHeartBeat:function () {
		if (CrestronMobile.debug) {
			CF.log("CrestronMobile: send heartbeat updateComplete=" + CrestronMobile.updateComplete);
		}
		if (CrestronMobile.updateComplete) {
			/*
	...
	setLoadingMessageVisible:function (show) {
		if (CrestronMobile.loadingMessageVisible !== show) {
